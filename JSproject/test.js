let targetNumber = Array(10).fill(null);
const lockedDigits = Array(10).fill(false);  
...
function generateRandomPhoneNumber() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const digit = lockedDigits[i] ? targetNumber[i] : randomDigit();
        ...
        span.addEventListener('click', () => {
            if (!lockedDigits[i]) {
                lockedDigits[i] = true;
                targetNumber[i] = span.innerText; 
                span.classList.add('locked');
            }
        });

        phoneNumberDiv.appendChild(span);
    }
}
